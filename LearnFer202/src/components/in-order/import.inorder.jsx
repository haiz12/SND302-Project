...
const ImportInOrderExcelPage = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:9999/in/template/download', {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', 'donnhap_template.xlsx');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:9999/in/import', formData, {
        headers: {
...
export default ImportInOrderExcelPage;