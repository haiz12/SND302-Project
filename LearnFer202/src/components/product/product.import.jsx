...
const ImportProductExcelPage = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:9999/products/template/download', {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', 'sanpham_template.xlsx');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:9999/products/import', formData, {
        headers: {
      ...
      setError(err.response?.data?.message || 'Có lỗi xảy ra khi import dữ liệu');
        ...
        <Card>
          ...
          <div style={{ display: 'flex', gap: '32px' }}>
            <div style={{ flex: '1' }}>
              ...
              <Card title={<Title level={4}>2. Upload file đã điền thông tin</Title>}>
                <Alert
...
export default ImportProductExcelPage;