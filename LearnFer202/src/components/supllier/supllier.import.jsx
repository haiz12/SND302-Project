...
const ImportSupllierExcelPage = () => {
  ...
  const handleDownloadTemplate = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:9999/suppliers/template/download', {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', 'nhacungcap_template.xlsx');
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:9999/suppliers/import', formData, {
        headers: {
...
export default ImportSupllierExcelPage;